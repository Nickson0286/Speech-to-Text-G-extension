window.addEventListener("load", function () {
  ...
  var permissionPageLink = document.querySelector(
    'a[href^="chrome://settings/content/siteDetails"]'
  );
  permissionPageLink.href = permissionPageLink.href.replace(
    "{{id}}",
    chrome.runtime.id
  );
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
    navigator.mediaDevices.enumerateDevices().then(function (devices) {
      ...
      var microphoneDevices = devices.filter(function (device) {
      ...
      .then(function (permissionStatus) {
        if (permissionStatus.state !== "granted") {
          ...
          if (document.getElementById("permissionRequestMessage")) {
            document.getElementById("permissionRequestMessage").style.display =
              "block";
          }
        }
      });
